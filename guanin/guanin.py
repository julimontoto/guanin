...
def RUVgnorm2(
    ...
    if center:
        Ycenter = Y.apply(
            lambda col: scale(col, with_mean=True, with_std=False), axis=0
        )
    else:
        Ycenter = Y
    ...
    svdWau, svdWad, svdWauv = svd(
        Ycenter.loc[:, gene_matrix.index.isin(refgenes)], full_matrices=False
    )
    first = 1 + drop
    k = min(
        k,
        max([i for i, val in enumerate(svdWad) if val > tolerance], default=0),
    )
    W = svdWau[:, first - 1:k]
